# Speak Stage English — конвейер роликов-суфлёров для уверенной речи

Название канала и хендл задаются в `config.json` → `brand`. Всё, что видно зрителю
(описание, подпись, обложка), берётся оттуда — правь только там.
